...
!
! Description: Interface between NWPSAF 1DVar code and RTTOV11
!
! History:
!
! Ticket  Date     Comment
...
INCLUDE 'NWPSAF_RTTOV11_Allocate.interface'
INCLUDE 'NWPSAF_RTTOV11_GetHMatrix.interface'
...
TYPE(profile_type)                  :: Profiles_K_PC(NPCScores)
...
ELSE RTTOV_FastmodelMode
  ...
  Profiles(1)%snow_frac = 0.0 ! Default to no snow
...
!      CALL rttov_print_profile(profiles_k_pc(1), lu=6)
    ...
    IF (Use_EmisAtlas .AND. Profiles(1)%skin%surftype /= RTsea) THEN
#ifdef _CompileEmissAtlas
      CALL rttov_deallocate_emis_atlas( RT_Coefs(Instrument) )
#endif
    END IF
