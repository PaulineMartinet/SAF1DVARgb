...
LOGICAL, parameter :: add_error=.true.
...
CHARACTER(len=200) :: outfile='/data/local/frfh/NWPSAF_1DVar/r1508_HTFRTC_LHR/1DVar/output_simrad_rt11/70L_200_PC.dat'
...
Type(profile_type), allocatable    :: profiles(:)         ! Atmospheric profiles
Type(radiance_type)                :: radiance            ! radiance structure
Type(rttov_pccomp)                 :: pccomp              ! PC scores
Type(transmission_type)            :: transmission        ! transmission structure
...
options(:) % rt_mw % fastem_version     = 6 ! **** remember to update this to  6 ****
...
options(:) % interpolation % interp_mode = 5 ! **** remember to update this to 5 ****
