...
!------------------------------------------------------------------------------
! Description: Read in Observations for 1DVar, convert BTs to Radiances, or
!              read in radiances from file, then convert to PCs and write them 
!              out again.
...
!General
INTEGER :: i,j,ii
INTEGER :: dim_evec_1, dim_evec_2

!Input/Output file info and storage variables
CHARACTER(LEN=*), PARAMETER  :: infile = &
  '/data/local/frfh/NWPSAF_1DVar/r1508_HTFRTC_LHR/1DVar/output_simrad_rt11/70L_8461_RR_truth.dat'
...
INTEGER             :: Err
INTEGER, PARAMETER  :: NPCScores=200
...
REAL, ALLOCATABLE :: evec_T(:,:)

LOGICAL :: convert_to_rad
...
Write(outunit,'(a)') 'Generated by sim_rad_iasi_rttov12_70L.F90, using PCRTTOV on 101 levels.'
Write(outunit,'(a)') 'Atmospheric state profiles were taken from the Met Office'
Write(outunit,'(a)') 'Truth file Model_20110315_1200_Sea.dat on 70L interpolated inside RTTOV10.'
Write(outunit,'(a)') ''
Write(outunit,'(a)') ''
Write(outunit,'(a)') 'The observations have been converted to PC space using PC-RTTOV coeffs'
Write(outunit,'(a)') 'using the program rad_to_pc.f90'
Write(outunit,'(a)') ''
Write(outunit,'(a)') ''
...
WRITE(*,*) 'Transposing eigenvectors'
dim_evec_1=SIZE(coef % coef_pccomp % eigen(1) % eigenvectors(:,:), 1)
dim_evec_2=SIZE(coef % coef_pccomp % eigen(1) % eigenvectors(:,:), 2)
ALLOCATE(evec_T(dim_evec_2,dim_evec_1))
evec_T=TRANSPOSE(coef % coef_pccomp % eigen(1) % eigenvectors(:,:))
...
IF (NumChannels /= 8461) CALL error_trap(' Need all channels for PC conversion')
...
IF (String(1:5) == 'Units' ) THEN
  ...
  IF (ReadStatus == 0) THEN
    IF ( VarChar(1:8) == 'PC Score' ) THEN
      Write(*,*) 'Observations are already PC scores!'
      STOP
    ELSE IF( VarChar(1:8) == 'Radiance' ) THEN 
...
ALLOCATE(Channel_Number(NumChannels))
Channel_Number(:) = 0
...
Write(outunit,'(a,i8)') 'No. of Chans per Observation:',NPCScores
...
Write(outunit,'(a)') 'Units: PC Score'
Write(outunit,'(a)') 'Sat. Series   Platform   Instrument First_Channel   Last_Channel  Sat ID'
Write(outunit,'(i5,i13,i12,i12,i16,i12)') Series,Platform,Subtype,1,NPCScores,WMO
Write(outunit,'(a)') 'PCs'
Write(outunit,'(16i5)') (/ (i,i=1,NPCscores) /)
Write(outunit,'(a)') '----------------------------------------------------------------------'
...
ReadData: DO WHILE (obnumber < NumObs)
  ...
  !-------
  IF (convert_to_rad) THEN
    READ (UNIT=inunit, FMT=*, IOSTAT=ReadStatus) Observation% BriTemp(:)
  ELSE
    READ (UNIT=inunit, FMT=*, IOSTAT=ReadStatus) Observation% Radiances(:)
  END IF
...
!--------------------------------------------------------
  ...
  IF ( convert_to_rad ) THEN
    ...
    Observation%Radiances(:) = &
      coef%coef%planck1(:) / ( EXP( coef%coef%planck2(:) / &
      ( Observation%BriTemp(:)+coef%coef%ff_bco(:)/coef%coef%ff_bcs(:))) - 1 )
  END IF
...
!--------------------------------------------------------
  ...
  WRITE(*,*) 'Calculating PCs'

  DO i=1, NPCScores
    DO j=1, NumChannels
      Observation%PC_scores(i)= Observation%PC_scores(i) + &
        evec_T(i,j) * &
        Observation%Radiances(j) / coef % coef_pccomp%noise_in(j)
    END DO
  END DO
...
!--------------------------------------------------------
  ...
  write(outunit,'(a)') 'PC Scores:'
  write(outunit,'(6e13.5)') Observation%PC_scores(:)
